...
from bsgnn.features import load_features
...
def get_data(path: str,
    ...
    # Load data
    with open(os.path.join(path, f'{args.dataset_name}.csv')) as f:
        reader = csv.reader(f)
    ...
    if os.path.exists(os.path.join(path, 'graph_cache.pickle')):

        with open(os.path.join(path, 'graph_cache.pickle'), 'rb') as f:
            all_data = pickle.load(f)

    else:
        all_data = {crystal_name: Poscar.from_file(os.path.join(path, 'poscar', crystal_name)).as_dict()
                    for crystal_name in tqdm(os.listdir(f'{path}/poscar/'))}
        with open(os.path.join(path, 'graph_cache.pickle'), 'wb') as f:
...
def split_data(data: MoleculeDataset,
    ...
    #     return scaffold_similarity_split(data, sizes=sizes, balanced=True, seed=seed, logger=logger)
