...
from bsgnn.data.utils import get_task_names, get_data, get_class_sizes, split_data
from bsgnn.data.scaler import StandardScaler
from bsgnn.nn_utils import param_count
from bsgnn.utils import build_optimizer, build_lr_scheduler, get_loss_func, get_metric_func, load_checkpoint, \
...
def run_training(crystal, train_data, val_data, test_data, fold_num, args: Namespace, logger: Logger = None) -> Tuple[List[Union[float, np.ndarray]], List[float]]:
    ...
    for model_idx in range(args.ensemble_size):
        ...
        for epoch in range(args.epochs):
            ...
            #plt.savefig("/GPUFS/nscc-gz_pinchen2/superconductors/test/cgcmpnn/bsgnn/"+str(model_idx)+"_"+str(epoch)+".png")
        ...
        if args.dataset_type == 'regression':
            ...
            df_test_result.to_csv(r'./result/df_test_result_regression' + str(fold_num) +'.csv')
        else:
            df_test_result = pd.DataFrame()
            for i,smile in enumerate(test_data.smiles()):
                df_test_result.loc[smile,'true'] = np.array(test_targets)[i][0]
                df_test_result.loc[smile,'pred'] = np.array(test_preds)[i][0]
                # df_test_result.loc[smile,'cf'] = crystal.loc[smile,'cf']
            df_test_result.to_csv(r'./result/df_test_result_classification' + str(fold_num) +'.csv')
